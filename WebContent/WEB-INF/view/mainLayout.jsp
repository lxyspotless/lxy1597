...
<body id="bodyContainer" class="easyui-layout">
	<div data-options="region:'north',border:false" style="height:15%;background:#A9FACD;padding:10px">north region</div>
				...
				<!--<c:if test="${item.PARENT_CODE == null}">
					<c:if test="${status.first}">
						<div title="'${item.MENU_NAME}'" data-options="id:'${item.MENU_CODE}',title:'${item.MENU_NAME}'
							,selected:true" style="background:#84C1FF;text-align:center;font-size:18px;padding-top:10px;padding-bottom:10px;"></div>
					</c:if>
					<c:if test="${!status.first}">-->
						<!--<div title="'${item.MENU_NAME}'" data-options="id:'${item.MENU_CODE}',title:'${item.MENU_NAME}'"
							style="background:#84C1FF;text-align:center;padding-top:10px;padding-bottom:10px;"></div>
					</c:if>-->
				<!--</c:if>
				<c:if test="${item.PARENT_CODE != null}">-->
					<div class="hdz_icon">
